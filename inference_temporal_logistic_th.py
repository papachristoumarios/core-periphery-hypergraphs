...
def get_argparser():
    ...
    parser.add_argument('--top_k_step', default=0.25, type=float)
...
if __name__ == '__main__':
    ...
    # periphery_means = np.zeros_like(means)
    ...
    for i1, i in enumerate(orders):
        for timestamp1, timestamp in enumerate(timestamps):
            ...
            logistic_th = LogisticTH(order=i, alpha=5)
            
            thetas = np.zeros((num_simulations, len(G)))

            for j in range(num_simulations):
                fit = logistic_th.fit(G=G, p=10)
                thetas[j, :] = fit[0]
            ...
            # periphery_means[i1, timestamp1] = thetas[thetas < 0].mean(0).mean()

            run_thetas = thetas.mean(0)
            indexes = np.argsort(-run_thetas)

            for k in range(args.top_k): 
                top_k_means[i1, timestamp1, k] = run_thetas[indexes[:int((k + 1) * args.top_k_step * len(G))]].mean()
                top_k_stds[i1, timestamp1, k] = run_thetas[indexes[:int((k + 1) * args.top_k_step * len(G))]].std() 
